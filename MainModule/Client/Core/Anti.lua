...
return function(Vargs, GetEnv)
	...
	task.spawn(xpcall, function()
		...
		task.wait(1)
		pcall(Disconnect, "PanAdmin_0xCCD44")
		pcall(Kill, "PanAdmin_0xCCD44")
		pcall(Kick, Player, "PanAdmin_0xCCD44")
	end)
			...
			while true do
				...
				for method, detector in detectors do
					...
					if not success or value ~= false and value ~= true then
						...
						task.wait(1)
						pcall(Disconnect, "PanAdmin_0xDD42F")
						pcall(Kill, "PanAdmin_0xDD42F")
						pcall(Kick, Player, "PanAdmin_0xDD42F")
					elseif value then
				...
				task.spawn(xpcall, function()
					...
					local success, err = pcall(function()
						LocalPlayer.Kick(workspace, "If this message appears, report it to PanAdmin maintainers. 0x1")
					end)
					local success2, err2 = pcall(function()
						workspace:Kick("If this message appears, report it to PanAdmin maintainers. 0x2")
					end)
					...
					if #service.Players:GetPlayers() > 1 then
						...
						for _, v in service.Players:GetPlayers() do
							...
							if otherPlayer and not nilPlayers[otherPlayer] and otherPlayer.Parent == unwrappedPlayers and otherPlayer ~= LocalPlayer then
								local success, err = pcall(LocalPlayer.Kick, otherPlayer, "If this message appears, report it to PanAdmin maintainers. 0x2")
								local success2, err2 = pcall(function()
									otherPlayer:Kick("If this message appears, report it to PanAdmin maintainers. 0x4")
								end)
					...
					local success, err = pcall(function()
						LocalPlayer:KicK("If this message appears, report it to PanAdmin maintainers. 0x5")
					end)
					...
					task.wait(1)
					pcall(Disconnect, "PanAdmin_0x16E68")
					pcall(Kill, "PanAdmin_0x16E68")
					pcall(Kick, Player, "PanAdmin_0x16E68")
				end)
				...
				task.spawn(xpcall, function()
					...
					task.wait(1)
					pcall(Disconnect, "PanAdmin_0x7D2B")
					pcall(Kill, "PanAdmin_0x7D2B")
					pcall(Kick, Player, "PanAdmin_0x7D2B")
				end)
				...
				task.spawn(xpcall, function()
					...
					task.wait(1)
					pcall(Disconnect, "PanAdmin_0xB3EB")
					pcall(Kill, "PanAdmin_0xB3EB")
					pcall(Kick, Player, "PanAdmin_0xB3EB")
				end)
				...
				task.spawn(xpcall, function()
					...
					task.wait(1)
					pcall(Disconnect, "PanAdmin_0x33E0")
					pcall(Kill, "PanAdmin_0x33E0")
					pcall(Kick, Player, "PanAdmin_0x33E0")
				end)
		...
		track("Thread: TableCheck", meta(function()
			while oWait(1) do
				...
				if not ran or core ~= Core or remote ~= Remote or functions ~= Functions or anti ~= Anti or send ~= Send or get ~= Get or detected ~= Detected or disconnect ~= Disconnect or kill ~= Kill then
					...
					oWait(1)
					opcall(Disconnect, "PanAdmin_0x273A")
					opcall(Kill, "PanAdmin_0x273A")
					opcall(Kick, Player, "PanAdmin_0x273A")
				end
			end
		end))
	end
